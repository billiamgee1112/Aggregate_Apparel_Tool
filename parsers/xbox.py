...
class XboxGameStudiosParser(BaseParser):
    ...
    # This bypasses custom layout tags completely across Shopify.
    ...
    def parse_product(self, product_el: BeautifulSoup, base_url: str) -> tuple:
        ...
        # Reset local cache state on first-page run boundaries
        if "page=" not in base_url or "page=1" in base_url:
            self._seen_urls.clear()

        # Filter out gift card options and filter variations
        ...
        # 2. Walk up to locate the encompassing card wrapper container
        ...
        # Shopify Dawn uses `.card__heading` while legacy templates use `.product-card__title`
        name_el = (
            container.select_one(".card__heading")
            or container.select_one(".card-information__text")
        ...
        # Refined text-scanning fallback ignoring standard Shopify badge words
        if not product_name:
        ...
        # Final image tag alt-attribute lookup fallback
        ...
        # Filter out nodes resolved as overlay badge text
        if not product_name or product_name.lower().strip() in ["sale", "new", "sold out", "preorder"]:
            return "", 0.0, None, "", "", {}
        ...
        # Dawn theme structures prioritize price tags with specialized classes
        price_el = (
        ...
        # 5. Extract Image Thumbnail (with Shopify CDN URL scheme sanitization)
        img_el = container.select_one("img")
        raw_image_url = ""
        if img_el:
