...
class BlizzardParser(BaseParser):
    brand_name = "Blizzard"
    # Shopify standard collections pagination works perfectly backend-side 
    # even when frontend rendering uses a load-more button mechanism!
    url_pattern = "https://gear.blizzard.com/collections/apparel?page={page_num}"
    first_page_override = "https://gear.blizzard.com/collections/apparel"
    ...
    def parse_product(self, product_el: BeautifulSoup, base_url: str) -> tuple:
        ...
        # Reject duplicate matches, checkout variations, and gift cards
        ...
        # 2. Walk up to locate the encompassing product card container
        ...
        name_el = (
            ...
            or container.select_one(".card__heading")  # Standard for newer Shopify themes
            or container.select_one(".product-item__title")
            or container.select_one(".title")
            or container.select_one("[class*='title']")
            or container.select_one("[class*='name']")
        )
        if name_el:
            product_name = name_el.get_text(strip=True)
            
        # Refined text-scanning fallback ignoring standard badge labels
        if not product_name:
            raw_text = product_el.get_text(" ", strip=True)
            badge_texts = ["sale", "new", "sold out", "preorder", "view"]
        ...
        # Reject empty or residual dummy tags
        if not product_name or product_name.lower().strip() in ["sale", "new", "sold out", "preorder"]:
            return "", 0.0, None, "", "", {}

        # Strip standard suffix labels
        if " - Blizzard Gear Store" in product_name:
            product_name = product_name.replace(" - Blizzard Gear Store", "").strip()
